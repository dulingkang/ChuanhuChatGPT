...
def get_html(filename):
    path = os.path.join(shared.sdb_path, "web_assets", "html", filename)
...
def webpath(fn):
    if fn.startswith(shared.assets_path):
        web_path = os.path.relpath(fn, shared.sdb_path).replace('\\', '/')
    else:
...
def list_scripts(scriptdirname, extension):
    scripts_list = []
    scripts_dir = os.path.join(shared.sdb_path, "web_assets", scriptdirname)
...
def reload_javascript():
    ...
    meta = """
        <meta name="apple-mobile-web-app-title" content="Resume Chat">
        <meta name="apple-mobile-web-app-capable" content="yes">
        <meta name="application-name" content="Resume Chat">
