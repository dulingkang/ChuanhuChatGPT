...
function addSdbButton(botElement) {
    ...
    if (!rawMessage) { // 如果没有 raw message，说明是早期历史记录，去除按钮
        // var buttons = botElement.querySelectorAll('button.sdb-btn');
    ...
    copyButton.classList.add('sdb-btn');
    ...
    toggleButton.classList.add('sdb-btn');
...
function setLatestMessage() {
    var latestMessage = gradioApp().querySelector('#sdb-chatbot > .wrapper > .wrap > .message-wrap .message.bot.latest');
...
function addLatestMessageButtons(botElement) {
    ...
    regenerateButton.classList.add('sdb-btn');
    ...
    deleteButton.classList.add('sdb-btn');
    ...
    likeButton.classList.add('sdb-btn');
    ...
    dislikeButton.classList.add('sdb-btn');
